 ...
 *
 * The extension redirects immediately on model selection, so no API call
 * ever goes to the aliases provider.
 */
...
export default function (pi: ExtensionAPI) {
	...
	// Redirect on user model selection
	pi.on("model_select", async (event, ctx) => {
		if (event.model.provider !== "alias") return;
		await resolveAlias(event.model.id, ctx);
	});

	// Safety net: resolve any lingering alias before the agent starts a turn
